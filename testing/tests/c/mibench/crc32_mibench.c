/*
 * CRC-32 MiBench — ASPIS adaptation.
 *
 * Original algorithm: Gary S. Brown, 1986 (public domain).
 * Source benchmark : MiBench, University of Michigan.
 *
 */
...
__attribute__((annotate("to_duplicate")))
static DWORD crc_accum;

static DWORD crc32buf(const char *buf, size_t len)
{
    crc_accum = 0xFFFFFFFF;
    for (; len; --len, ++buf)
        crc_accum = UPDC32(*buf, crc_accum);
    return ~crc_accum;
}
